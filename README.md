...
Home Assistant 自訂整合，支援樂奇電器（Lifegear）智慧果 M8 / M8-E 全熱交換機、浴室暖風機、M8-E 感測器。
...
- ✅ **支援 M8-E**（淨流系統）（v4.0.0+）
- ✅ **支援浴室暖風機 BD-125W / 智慧果 M8-E 感測器**（v4.2.0+）
- ✅ **多設備自動發現**：登入後自動建立帳號下所有設備（v4.2.0+）
- ✅ **濾網提醒**：使用時數、更換提醒、重置（v4.2.0+）
- ✅ M8 裝置連線狀態偵測
- ✅ 手動重新登入按鈕
...
| 隱藏式新風機 2.0 (M8-E) | 樂奇淨流系統 | 淨化/新風/節能 | 弱/中/中強/強 |
| 浴室暖風機 BD-125W | 樂奇淨流系統 | 涼風/換氣/乾燥-節電/乾燥-快速/暖房-沐浴/暖房-溫控 | 弱/中/強 |
| 智慧果 M8-E 感測器 | 樂奇淨流系統 | 空氣品質偵測（CO2/PM2.5/溫度/濕度） | — |
...
前往 **設定** → **裝置與服務** → **新增整合** → 搜尋 **Lifegear** 或 **樂奇**，選擇設備型號和登入方式：

- **智慧果 M8**：選擇「智慧果 M8」
- **淨流系統設備**（M8-E / 暖風機 / M8-E 感測器）：選擇「淨流系統」，登入後帳號下所有設備會自動建立

### 方式一：本地控制（推薦）

搭配 `m8_local_server` add-on，感測器資料完全在本地讀取，不依賴雲端穩定性。

#### 安裝 m8_local_server add-on

<details>
<summary>展開安裝步驟</summary>

**1. 複製 add-on 檔案到 HAOS**

透過 Samba share 或 SSH，將本 repo 的 [`addon/`](addon/) 資料夾複製到 HAOS 的 `/addons/local/m8_local_server/`，同時將 [`m8_local_server.py`](m8_local_server.py) 也複製進去：

```
/addons/local/m8_local_server/
├── config.json
├── Dockerfile
├── run.sh
└── m8_local_server.py
...
前往 **設定** → **附加元件** → 右上角 **...** → **檢查更新** → 找到 **M8 Local Server** → **安裝** → **啟動**

**3. 設定網路 DNAT（以 UniFi UDM Pro 為例）**

將 M8 裝置發往雲端伺服器的 HTTP 流量攔截並導向 HAOS：

- **DNAT 規則**：M8 裝置 IP → 雲端 IP (`61.31.209.215`) port 80 TCP → 轉發至 HAOS IP port 80
- **防火牆規則**：允許 M8 裝置 IP → HAOS IP（所有協定）

> 如果 M8 和 HAOS 在不同 VLAN，需要額外的跨 VLAN 防火牆規則。

**4. 確認 add-on 運作**

瀏覽器開啟 `http://<HAOS IP>:8765/api/status`，應看到 M8 的感測器資料（M8 需等幾秒開始回報）。
...
|------|------|------|
| CO2 | 二氧化碳濃度 (ppm) | 全部 |
| PM2.5 | 細懸浮微粒濃度 (µg/m³) | 全部 |
| 溫度 | 室內溫度 (°C) | M8 / M8-E / M8-E 感測器 |
| 濕度 | 室內相對濕度 (%) | 全部 |
| 目前風速 | 當前風速檔位 | M8 / M8-E |
| 目前模式 | 當前運轉模式 | M8 / M8-E |
| 目前功能 | 當前功能名稱 | 暖風機 |
| 連線狀態 | 裝置是否在線 | 全部 |
| 高效濾網已使用 | 高效濾網累計使用時數 | M8-E |
| 初效濾網已使用 | 初效濾網累計使用時數 | M8-E / 暖風機 |

### 控制

| 實體 | 說明 | 設備 |
|------|------|------|
| 電源 | 開關機控制 | M8 / M8-E / 暖風機 |
| 模式 | M8：自動/淨化/全熱；M8-E：淨化/新風/節能 | M8 / M8-E |
| 風速 | M8：1-4 檔滑桿；M8-E：弱/中/中強/強 | M8 / M8-E |
| 功能 | 涼風/換氣/乾燥-節電/乾燥-快速/暖房-沐浴/暖房-溫控 | 暖風機 |
| 風速 | 弱/中/強 | 暖風機 |
| 倒數關機 | 5-480 分鐘（乾燥快速/暖房沐浴/暖房溫控最長 180 分鐘） | 暖風機 |
| 高效濾網更換提醒 | 3000-8000 小時 | M8-E |
| 初效濾網更換提醒 | 720/1440/2160 小時 | M8-E / 暖風機 |
| 高效濾網重置 | 重置累計使用時數 | M8-E |
| 初效濾網重置 | 重置累計使用時數 | M8-E / 暖風機 |
| 重新登入 | 手動刷新 AuthCode（有設定帳密時才出現） | 全部 |
...
### v4.2.0
- 新增：支援浴室暖風機 BD-125W（涼風/換氣/乾燥/暖房六種功能、風速弱/中/強、倒數關機）
- 新增：支援智慧果 M8-E 獨立感測器（CO2/PM2.5/溫度/濕度）
- 新增：多設備自動發現 — 淨流系統帳號登入後自動建立所有設備
- 新增：濾網提醒 — 使用時數感測器、更換提醒時數設定、濾網重置按鈕
- 改善：雲端輪詢間隔調整為 60 秒，濾網資料 30 分鐘查詢一次，降低伺服器負載
- 改善：reconfigure / options flow 正確保留設備類型和 MAC

### v4.1.0
- 新增：M8-E 完整支援（淨化/新風/節能模式、電源獨立控制、感測器讀取）
- 新增：M8-E 切模式/風速時，設備關機會自動開機
- 改善：M8 連線狀態偵測改為基於時間判斷（30 秒無資料 = 離線）
- 修正：M8-E API 回傳格式差異（欄位名、回應結構、endpoint）

### v4.0.0
- 新增：支援 M8-E（淨流系統 App），設定時選擇設備型號
- M8-E 使用不同 API endpoint，加密方式與 M8 相同
